...
struct State {
    ...
    void move(const libataxx::Move &move) {
        pos.makemove(move);
    }
    ...
    State(libataxx::Position position) : pos(position) {
        num_moves = position.legal_moves(moves);
        curr_move = 0;
    }
};
...
struct Node {
    ...
    Node *expand() {
        ...
        libataxx::Move nmove = state.next();
        libataxx::Position npos = state.pos;
        npos.makemove(nmove);

        State nstate = State(npos);
        Node child = Node(nstate, this, nmove);
    ...
    Node(State s, Node *p, libataxx::Move m)
...
Node *best_child(Node *node, const float c) {
    ...
    for (Node &child : node->children) {
        ...
        float captures = (float)(count_captures(node->state.pos, child.move) +
                                 (child.move.is_single() ? 1 : 0)) /
                         10.0;
...
float default_policy(libataxx::Position &pos) {
    float r = 0.0;
    for (int i = 0; i < 5; ++i) {
...
PV get_pv(Node *node) {
    ...
    while (node->children.size() > 0) {
        ...
        // libataxx::Move on to the best child node
...
void print_tree(Node node, int depth) {
    ...
    for (auto &child : node.children) {
        ...
        std::cout << child.move << " (" << child.score << "/" << child.visits
                  << " " << score << ")" << std::endl;
...
void mcts_uct(const libataxx::Position &pos, int nodes, int movetime) {
    Node root = Node(pos, NULL, libataxx::Move::nullmove());
    int iteration = 0;
    ...
    while (iteration < nodes && clock() < end_target) {
        ...
        if (iteration == 1 || iteration % 1000 == 0) {
            ...
            std::cout << " time " << (uint64_t)(1000.0 * time) << " pv "
                      << pv_string << std::endl;
        }
    }
    ...
    if (pv.num_moves > 0) {
        std::cout << "bestmove " << pv.moves[0] << std::endl;
    } else {
        // Play a random move
        libataxx::Move moves[256];
